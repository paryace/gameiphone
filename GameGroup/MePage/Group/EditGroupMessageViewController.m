...
@interface EditGroupMessageViewController ()
{
...
- (void)viewDidLoad
{
    ...
    switch (self.editType) {
        ...
        {
            [self setTopViewWithTitle:@"群昵称" withBackButton:YES];
            m_maxZiShu = 10;
        } break;
...
- (void)okButtonClick:(id)sender
{
    ...
    if (self.delegate &&[self.delegate respondsToSelector:@selector(comeBackInfoWithController:type:info:)]) {
        [self.delegate comeBackInfoWithController:self type:self.editType info:m_contentTextView.text];
    }
