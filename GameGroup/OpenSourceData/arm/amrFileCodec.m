...
NSData* EncodePCMToAMR(char* data, int maxLen,int nChannels, int nBitsPerSample,NSString *title)
{
    char* oldBuf = data;
    /* input speech vector */
	short speech[160];
    ...
    NSString *wavFile        = [documentPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.amr",title]];
    NSLog(@"%@",wavFile);
...
NSData* EncodeWAVEToAMR(NSData* data, int nChannels, int nBitsPerSample,NSString *title)
{
    NSArray *paths               = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentPath       = [paths objectAtIndex:0];
    ...
    return EncodePCMToAMR(buf,maxLen- nPos,nChannels,nBitsPerSample,title);
}
